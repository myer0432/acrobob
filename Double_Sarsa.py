...
def main():
    ...
    table_1 = QTable(LOWER_BOUNDS, UPPER_BOUNDS, BINS, OFFSETS, ACTIONS)
    table_2 = QTable(LOWER_BOUNDS, UPPER_BOUNDS, BINS, OFFSETS, ACTIONS)
    ############
    # Training #
    ############
    ...
    for ttrial in ttrials:
        ...
        for step in steps:
            ...
            if observation is None: # If this is the first action
                ...
                action = table_1.chooseDoubleAction(table_2,state, EPSILON)
            ...
            if flip == 0:
                table_1.double_update(table_2, previous_state, action, state, reward, ALPHA, GAMMA)
            elif flip == 1:
                table_2.double_update(table_1, previous_state, action, state, reward, ALPHA, GAMMA)
    ...
    plt.axes([.1,.1,.8,.7])
    plt.figtext(.5,.9,"Double SARSA Performance", fontsize=18, ha="center")
